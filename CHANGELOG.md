...
## 1.16.0 (2026-07-04)
  ...
  * Reorder the GPU block to load, power, clock, temp so it runs
    parallel to the CPU block (load, power, freq). Power now sits in
    the same slot in both blocks.
  * Move the commented cpu_temp stub to the end of the CPU block so
    uncommenting it later places temp last, mirroring gpu_temp.
  * README: resync the Options table, the intro block order, and the
    example line to the new element order.
