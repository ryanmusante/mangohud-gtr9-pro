...
Toggle with `Shift_R + F12` (MangoHud default). Move the bar with `position=top-center` or `top-right`.
...
```
FPS 142 8.5 ms  GPU 96% 2901 MHz 74°C  CPU 38% 4102 MHz  VRAM 0.5 GiB  RAM 13.2 GiB
```
...
On this shared-memory APU `vram` shows only the small BIOS carveout, so `ram` is the figure to watch. `legacy_layout=0` keeps the on-screen order identical to the config.
